...
class Helldivers:
    ...
    def run(self):
        ...
        threads = []
        for _ in range(self.r):
            t = Thread(target=self.regular_citizen, daemon=True)
            t.start()
            threads.append(t)

        for _ in range(self.s):
            t = Thread(target=self.super_citizen, daemon=True)
            t.start()
            threads.append(t)

        assemble_team = Thread(target=self.assemble_team)
        assemble_team.start()
        threads.append(assemble_team)
